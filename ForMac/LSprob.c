    ...
    while((!flag) && (!flag_MRCA)){
       ...
       if(*(branchProb+i)<=0){
        ...
        if(nbranch == NumLineage){
			...
			fprintf(stderr, "Stopping criterion is met.\n");
			flag = 1;
        }
        else{
...
void FDupdate(long* TYPE, long* type2node, const double RHO, const double THETA, long k, long* ntype, long* nbranch, long* nanc, double* m, double* logw, tsk_table_collection_t* tables, int *rec_num, int *mut_num, int* coal_num, int* nonrec)
{
    ...
    long onode, onode2, nnode, nnode2, edge, mut, *edge_arr, nodeTime;
    ...
    if(e <= K*L){/*mutation*/
		...
		//fprintf(stderr, "Mutation event is chosen on node %ld, locus %ld.\n", onode, i);
		*mut_num += 1;
		...
		//sprintf(state, "%d", a);
		//mut = tsk_mutation_table_add_row(&tables->mutations, 1, onode, -1, state, 0, NULL, 0); 
